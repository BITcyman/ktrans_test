...
mkdir -p DeepSeek-R1-GGML-FP8-Hybrid
cd DeepSeek-R1-GGML-FP8-Hybrid

mamba activate ktransformers 
python -m ktransformers.server.main \
    --host 0.0.0.0 --port 36666 \
    --backend_type balance_serve \
    --model_name DeepSeek-R1-GGML-FP8-Hybrid \
    --model_path /mnt/data/models/DeepSeek-R1-GGML-FP8-Hybrid/config \
    --gguf_path /mnt/data/models/DeepSeek-R1-GGML-FP8-Hybrid \
    --cache_len 131072 \
    --max_new_tokens 8192 \
    --force_think \
...
for concurrent in 1 2 4 8; do
    for prompt_lens in 1024 2048; do
        for max_tokens in 128 512; do \
            log_file="testpy_concurrent_${concurrent}_prompt_lens_${prompt_lens}_max_tokens_${max_tokens}.log"
            
            # 运行 Python 脚本并重定向输出到对应的日志文件
